...
export async function cancelPendingSessionController(req: Request, res: Response){
    ...
    if (error instanceof SessionNotPending) {
        return res.status(400).json({ message: "A sessão não está pendente e não pode ser cancelada" });
    }

    if (error instanceof userIsNotMaster) {
        return res.status(403).json({ message: "Apenas o mestre da sessão pode cancelá-la" });
    }
